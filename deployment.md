...
(`DB_HOST`, `REDIS_URL`, the internal service hostports…). Five values are
marked `sync: false`, meaning Render asks you for them and — importantly —
never overwrites them on a later sync:
...
The last three are URLs Render cannot derive. Internal service-to-service
hostnames *are* derived — `render.yaml` reads them off each service with
`fromService: property: hostport`, so they stay correct no matter what Render
names things — but there is no equivalent property for a service's public
`onrender.com` URL, so those three have to be typed in.

**Get them from the dashboard, after the services exist.** Render appends a
suffix when a service name is already taken, so you may well end up with
`aegiszero-gateway-y415.onrender.com` rather than the plain name. Copy the
real URLs rather than assuming.

Render prompts for these during Apply. The three URLs are a chicken-and-egg
problem on a first deploy — the services do not exist yet, so their URLs are not
known. Put anything in, finish the Apply, then come back in Step 4 and set them
to the real values once the dashboard shows them.

To set one afterwards: *service → Environment → Add environment variable*, then
**Manual Deploy → Deploy latest commit** (or *Clear build cache & deploy* for
...
If you filled in the three URL variables during Step 2, this is just a
verification pass. Compare each against the real URLs now visible in the
dashboard.
...
cache & deploy*, not a restart. A restart will appear to do nothing.
